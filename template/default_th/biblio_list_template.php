...
function biblio_list_format($dbs, $biblio_detail, $n, $settings = array(), &$return_back = array()) {
  ...
  $output .= '<a class="btn btn-outline-primary btn-block mt-2 btn-sm" href="'.$detail_url.'">ดูรายละเอียด</a>';
  $output .= '<a class="btn btn-outline-secondary btn-block mt-2 btn-sm openPopUp citationLink" href="'.$cite_url.'" title="'.str_replace('{title}', substr($title, 0, 50) , __('อ้างอิงสำหรับ: {title}')).'" target="_blank">อ้างอิง</a>';
  $output .= '</div>'; // -- close col-2
